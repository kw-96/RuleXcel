...
class VisualRuleEngine {
    ...
    bindEvents() {
        ...
        document.getElementById('save-rules-template')?.addEventListener('click', () => this.saveRulesTemplate());
        document.getElementById('load-rules-template')?.addEventListener('click', () => this.loadRulesTemplate());
    }
    ...
    saveRulesTemplate() {
        ...
        localStorage.setItem('ruleXcel_rules_template', JSON.stringify(template));
        
        // 显示成功提示
        this.showNotification('规则模板已保存', 'success');
    }
     ...
     */
    loadRulesTemplate() {
        const templateStr = localStorage.getItem('ruleXcel_rules_template');
        if (!templateStr) {
            this.showNotification('暂无保存的规则模板', 'warning');
            return;
        }
        
        try {
            const template = JSON.parse(templateStr);
            this.loadRulesFromTemplate(template);
            this.showNotification('规则模板已加载', 'success');
        } catch (error) {
            this.showNotification('规则模板格式错误', 'error');
        }
    }

    /**
     * 从模板加载规则
     * @param {Object} template - 规则模板
     */
    loadRulesFromTemplate(template) {
        // 清空现有规则
        this.clearAllRules();
        
        // 加载筛选条件
        if (template.filterConditions) {
            template.filterConditions.forEach(condition => {
                this.addFilterCondition();
                // 填充条件数据
                const conditionElement = document.querySelector('.filter-condition:last-child');
                this.fillFilterCondition(conditionElement, condition);
            });
        }
        
        // 加载排序条件
        if (template.sortConditions) {
            template.sortConditions.forEach(condition => {
                this.addSortCondition();
                // 填充条件数据
                const conditionElement = document.querySelector('.sort-condition:last-child');
                this.fillSortCondition(conditionElement, condition);
            });
        }
        
        // 加载合并配置
        if (template.mergeConfig) {
            document.getElementById('merge-type').value = template.mergeConfig.type;
            this.handleMergeTypeChange(template.mergeConfig.type);
        }
    }

    /**
     * 填充筛选条件数据
     * @param {HTMLElement} element - 条件元素
     * @param {Object} condition - 条件数据
     */
    fillFilterCondition(element, condition) {
        element.querySelector('.filter-column-select').value = condition.column;
        element.querySelector('.operator-select').value = condition.operator;
        element.querySelector('.value-type-select').value = condition.valueType;
        
        if (condition.logical) {
            const logicalSelect = element.querySelector('.logical-select');
            if (logicalSelect) {
                logicalSelect.value = condition.logical;
            }
        }
        
        // 触发值类型变化事件
        this.handleValueTypeChange(element.id, condition.valueType);
        
        // 设置值
        setTimeout(() => {
            if (condition.valueType === 'column') {
                const columnSelect = element.querySelector('.column-value-select');
                if (columnSelect) {
                    columnSelect.value = condition.value;
                }
            } else {
     ...
     * @param {Object} condition - 条件数据
     */
    fillSortCondition(element, condition) {
        element.querySelector('.sort-column-select').value = condition.column;
        element.querySelector('.sort-direction-select').value = condition.direction;
    }
    ...
    getMergeConfig() {
        ...
        const config = { type: mergeType };
        
        switch (mergeType) {
            case 'files':
                config.method = document.getElementById('file-merge-method')?.value;
                config.joinColumn = document.getElementById('join-column')?.value;
                break;
            case 'sheets':
                config.method = document.getElementById('sheet-merge-method')?.value;
                break;
            case 'columns':
                config.joinType = document.getElementById('join-type')?.value;
                config.joinColumn = document.getElementById('join-key-column')?.value;
                break;
        }
    ...
    /**
     * 生成Arquero查询
     * @param {Object} data - 数据表
     * @returns {Object} 处理后的数据表
     */
    applyRules(data) {
        let result = data;
        ...
        if (filterConditions.length > 0) {
            result = this.applyFilterRules(result, filterConditions);
        }
        
        // 应用排序规则
        const sortConditions = this.getSortConditions();
        if (sortConditions.length > 0) {
            result = this.applySortRules(result, sortConditions);
        }
     ...
     * @param {Object} data - 数据表
     * @param {Array} conditions - 筛选条件
     * @returns {Object} 筛选后的数据表
     */
    applyFilterRules(data, conditions) {
        // 构建Arquero筛选函数
        const filterFunction = this.buildArqueroFilterFunction(conditions);
     ...
     * @param {Object} data - 数据表
     * @param {Array} conditions - 排序条件
     * @returns {Object} 排序后的数据表
     */
    applySortRules(data, conditions) {
        const sortColumns = conditions.map(condition => {
            return condition.direction === 'desc' ? `-${condition.column}` : condition.column;
        });
        
        return data.orderby(...sortColumns);
    }

    /**
     * 构建Arquero筛选函数
     * @param {Array} conditions - 筛选条件
     * @returns {Function} Arquero筛选函数
     */
    buildArqueroFilterFunction(conditions) {
        return (d) => {
            let result = true;
            let currentResult = null;
            
            conditions.forEach((condition, index) => {
                const columnValue = d[condition.column];
                ...
                switch (condition.operator) {
                    case '==':
                        if (condition.valueType === 'column') {
                            conditionResult = columnValue == d[condition.value];
                        } else {
                            conditionResult = columnValue == condition.value;
                        }
                        break;
                    case '!=':
                        if (condition.valueType === 'column') {
                    ...
                    case '>':
                        if (condition.valueType === 'column') {
                            conditionResult = Number(columnValue) > Number(d[condition.value]);
                        } else {
                            conditionResult = Number(columnValue) > Number(condition.value);
                        }
                        break;
                    case '>=':
                        if (condition.valueType === 'column') {
                    ...
                    case 'contains':
                        if (condition.valueType === 'column') {
                            conditionResult = String(columnValue).includes(String(d[condition.value]));
                        } else {
                            conditionResult = String(columnValue).includes(String(condition.value));
                        }
                        break;
                    case 'not_contains':
                        if (condition.valueType === 'column') {
                    ...
                    case 'starts_with':
                        if (condition.valueType === 'column') {
                            conditionResult = String(columnValue).startsWith(String(d[condition.value]));
                        } else {
                            conditionResult = String(columnValue).startsWith(String(condition.value));
                        }
                        break;
                    case 'ends_with':
                        if (condition.valueType === 'column') {
                            conditionResult = String(columnValue).endsWith(String(d[condition.value]));
                        } else {
                            conditionResult = String(columnValue).endsWith(String(condition.value));
                        }
                        break;
                    case 'is_empty':
                        conditionResult = !columnValue || columnValue === '';
                        break;
                    case 'is_not_empty':
                        conditionResult = columnValue && columnValue !== '';
                        break;
