...
class QuickActions {
    ...
    _initializeActions() {
        ...
        this.actions.set('extractQualityResources', {
            ...
            title: '提取优质资源位',
            description: '筛选出评分高、表现好的优质资源位',
            icon: '⭐',
            category: 'filter',
            qualityColumns: ['评分', '点击率', '转化率', 'score', 'rating', 'click_rate', 'conversion_rate'],
     ...
     * @returns {Object} 处理结果
     */
    async extractQualityResources(data, options = {}) {
        try {
            logger.info('开始提取优质资源位', { rows: data.length });
            ...
            // 查找质量评估列
            const qualityColumn = this._findQualityColumn(data[0], this.actions.get('extractQualityResources').qualityColumns);
            
            if (!qualityColumn) {
                throw new Error('未找到有效的质量评估列，支持的列名：' + this.actions.get('extractQualityResources').qualityColumns.join(', '));
            }

            // 确定质量阈值（默认取前30%）
            const threshold = options.threshold || 0.7;
            const qualityValues = data
                .map(row => parseFloat(row[qualityColumn]))
                .filter(val => !isNaN(val))
                .sort((a, b) => b - a);

            if (qualityValues.length === 0) {
                throw new Error(`质量列"${qualityColumn}"中没有有效的数值数据`);
            }

            const thresholdValue = qualityValues[Math.floor(qualityValues.length * (1 - threshold))];

            // 筛选优质资源
            const qualityData = data.filter(row => {
                const value = parseFloat(row[qualityColumn]);
                return !isNaN(value) && value >= thresholdValue;
            });

            logger.info('优质资源位提取完成', { 
                originalRows: data.length,
                qualityRows: qualityData.length,
                qualityColumn,
                thresholdValue 
            });
            ...
            return {
                success: true,
                data: qualityData,
                message: `提取出 ${qualityData.length} 个优质资源位（${qualityColumn} >= ${thresholdValue.toFixed(2)}）`,
                stats: {
    ...
    async executeAction(actionId, data, extraParam = null, options = {}) {
        ...
        // 对于筛选本月数据操作，传递额外的月份参数
        if (actionId === 'filterCurrentMonth' && extraParam) {
